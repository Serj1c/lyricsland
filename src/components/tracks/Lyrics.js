...
class Lyrics extends Component {
    ...
    componentDidMount() {
        ...
        commontrack_id=${this.props.match.params.id}&apikey=${process.env.REACT_APP_MM_KEY}`)
            .then(res => {
    ...
    render() {
        ...
        if(lyrics === undefined || Object.keys(lyrics).length === 0) {
            ...
            return (
                <React.Fragment>
                    <Link to="/" className="btn btn-dark mb-4 btn-sm">Back</Link>
                    <div className="card-body">
                        <p className="card-text">{this.state.lyrics.lyrics_body}</p>
                        <p className="card-text">{this.state.lyrics.lyrics_copyright}</p>
                    </div>
                </React.Fragment>
            )
        }
